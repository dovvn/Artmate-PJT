...
<<<<<<< HEAD
	...
	public String insertUser(@RequestBody UserDto user) {
		user.setAccesskey(makeAccessKey());
		if(uservice.selectUserId(user.getUserId()) == null) {
			uservice.insertUser(user); //아이디 중복확인 후 회원가입
			return user.getAccesskey();
		}
		else return "fail";//아이디 중복
=======
	@Autowired
...
>>>>>>> develop
	...
	private String makeAccessKey() { 
		...
		for(int i=0; i<10; i++) sb.append(randomString.charAt(random.nextInt(randomString.length())));
		return sb.toString();
	}
<<<<<<< HEAD
=======
